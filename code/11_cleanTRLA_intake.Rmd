...
if(RUN_FROM_CONSOLE){
  ...
  DATA_DIR = paste0(user, "/Dropbox/qss20_finalproj_rawdata/summerwork")
}
```
...
#merge trla_orig data and trla_more
trla_merge <- trla_orig2 %>%
  ...
  mutate(
    ...
    #note source of AP
    derived_source2 = case_when(is.na(derived_opponent_consolidated) ~ NA_character_,
                                        id == "2" ~ "trla_more",
    ...
    derived_opponent_source = case_when((derived_opponent_source == "Missing all" & !is.na(derived_opponent_consolidated)) ~ "trla_more",
                                        ...
                                        (is.na(derived_opponent_source) & derived_source2=="trla_more") ~ "trla_more",
                                        TRUE ~ "Missing all"),
...
sum(trla_merge$count_AP_states.y[trla_merge$derived_source2 == "trla_more"], na.rm = TRUE)
```

```{r}

#Checking the additional rows added
View(trla_merge %>%
  filter(id == "2"))
...
trla_merge %>%
      group_by(derived_opponent_source) %>%
       summarise(n())

```

We'd want to separate out the AP names prior to this step -- there would only be ~250 to separate.

Separate states & filter out government agencies & intakes missing AP:

```{r}
trla_merge2 <- trla_merge %>%
  
  mutate(derived_opponent_state = case_when(!is.na(derived_opponent_state) ~ derived_opponent_state,
                                            is.na(opstate.x) ~ opstate.y, 
                                            TRUE ~ opstate.x)) %>%
  
    separate_rows(derived_opponent_state, sep = ",")  %>%
  
  select(c("case_number","intake_date.x", "derived_opponent_consolidated", "derived_opponent_state", "derived_opponent_city", "missing_AP", "count_AP_states.y", "derived_is_notemp", "derived_opponent_source")) %>%
...
#TO DO
- Deal with multiple opps on one line in trla_more data
- Deal with incorrect data /  missing states when city is filled in
- See 174-178

Rebecca's older code:
```{r}
...
#flag for multiple case numbers
trla_orig2 = trla_orig2 %>%
        group_by(case_number) %>%
